...
export async function getRawBlock(height: number) {
  ...
  return await callRpc(z.string(), 'getblock', [hash, 0])
}
...
export async function getRawBlocks(heights: number[]): Promise<GetRawBlocksResponse> {
  ...
  const hashes = await callMultiRpc(z.string(), uniqueHeights.map(h => ['getblockhash', [h]] as const))
  ...
  const blocks = await callMultiRpc(z.string(), successfulHashes.map(h => ['getblock', [h.response, 0]] as const))
